-- Trimite către peer personajele competitive deblocate local. Verificarea
...
function characterAvailability.ReportIfChanged(liveIPC)
    ...
    for _, characterType in ipairs(characterCatalog.types) do
        ...
        Isaac.DebugString("[Isaac1v1P2P] CHARACTER_AVAILABILITY_DECISION player_type=" .. tostring(characterType)
    ...
    Isaac.DebugString("[Isaac1v1P2P] CHARACTER_AVAILABILITY types=[" .. signature .. "] names=[" .. table.concat(names, ",") .. "]")
    Isaac.DebugString("[Isaac1v1P2P] PLAYER_AVAILABILITY_REPORTED character_types=\"" .. signature .. "\" sent=\"" .. tostring(sent) .. "\"")
end
