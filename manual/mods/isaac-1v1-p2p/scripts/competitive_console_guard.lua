...
local function logBlocked()
    ...
    Isaac.DebugString('[Isaac1v1P2P] COMPETITIVE_CONSOLE_BLOCKED command="<toggle>"')
end

local function nativeDevConsoleUnlocked()
    -- API-ul există exclusiv într-un DLL compilat DEV; Production nu poate
    -- activa bypass-ul printr-un mesaj de transport sau printr-un apel Lua manual.
    if type(Isaac1v1P2P) ~= "table" or type(Isaac1v1P2P.IsDevConsoleUnlocked) ~= "function" then
        return false
    end
    local ok, unlocked = pcall(Isaac1v1P2P.IsDevConsoleUnlocked)
...
function consoleGuard.Register(mod, competitiveRun)
    ...
    if ModCallbacks == nil or ModCallbacks.MC_INPUT_ACTION == nil
        ...
        Isaac.DebugString('[Isaac1v1P2P] COMPETITIVE_CONSOLE_GUARD_UNAVAILABLE reason="INPUT_API_MISSING"')
        return false
    end
    -- Aceasta este doar o protecție suplimentară: consola ImGui REPENTOGON poate
    -- ocoli callback-ul. Extensia P2P nativă blochează comanda jucătorului direct
    ...
    Isaac.DebugString('[Isaac1v1P2P] COMPETITIVE_CONSOLE_GUARD_READY api="MC_INPUT_ACTION/ACTION_CONSOLE"')
    return true
end
