...
local function isLaunchableSource(source)
    return source == "STEAM_P2P"
end

local function unavailable(reason)
    status = "ERROR"
    lastError = reason
    Isaac.DebugString("[Isaac1v1P2P] RUN_LAUNCH_UNAVAILABLE reason=" .. quote(reason))
end

local function getCompetitiveSaveSlot()
    if type(Isaac1v1P2P) ~= "table"
        or type(Isaac1v1P2P.GetCompetitiveSaveSlot) ~= "function" then return "Unknown" end
    local ok, slot = pcall(Isaac1v1P2P.GetCompetitiveSaveSlot)
...
local function failConsumedLaunch(reason, detail, session, liveIPC, competitiveRun)
    ...
    unavailable(reason)
    Isaac.DebugString(
        "[Isaac1v1P2P] COMPETITIVE_LAUNCH_FAILED " ..
    ...
    if liveIPC ~= nil and liveIPC.CancelMatch ~= nil then
        ...
        Isaac.DebugString(
            "[Isaac1v1P2P] COMPETITIVE_LAUNCH_CANCEL_SENT " ..
            "match_id=" .. quote(session ~= nil and session.matchId or "Unknown") .. " " ..
...
local function startFromSession(session, gameState, matchBridge, liveIPC, liveStart, competitiveRun, modCompatibility)
    ...
    if liveIPC ~= nil and liveIPC.IsCancelled ~= nil then
        ...
        if cancelledOk and cancelled == true then
            Isaac.DebugString("[Isaac1v1P2P] RUN_START_CANCELLED_IGNORED match_id=" .. quote(session.matchId))
            return false
        end
    end
    ...
    Isaac.DebugString("[Isaac1v1P2P] RUN_LAUNCH_REQUEST_CONSUMED match_id=" .. quote(session.matchId))

    Isaac.DebugString(
        "[Isaac1v1P2P] RUN_LAUNCH_REQUESTED " ..
        "match_id=" .. quote(session.matchId or "Unknown") .. " " ..
        "character=" .. quote(session.characterName) .. " " ..
        "seed=" .. quote(session.seed)
    )
    Isaac.DebugString(
        "[Isaac1v1P2P] RUN_START_REQUESTED " ..
        "match_id=" .. quote(session.matchId or "Unknown") .. " " ..
    ...
    Isaac.DebugString(
        "[Isaac1v1P2P] COMPETITIVE_LAUNCH_BEGIN " ..
        "match_id=" .. quote(session.matchId) .. " slot=" .. quote(saveSlot)
    )
    if type(Isaac1v1P2P) ~= "table"
        or type(Isaac1v1P2P.PrepareCompetitiveSaveSlot) ~= "function" then
    ...
    local slotOk, slotPrepared, slotError = pcall(
        Isaac1v1P2P.PrepareCompetitiveSaveSlot, session.matchId)
    ...
    Isaac.DebugString(
        "[Isaac1v1P2P] START_NEW_GAME_CALL " ..
        "match_id=" .. quote(session.matchId) .. " slot=" .. quote(saveSlot)
    )
    ...
    Isaac.DebugString(
        "[Isaac1v1P2P] START_NEW_GAME_RETURN " ..
        "match_id=" .. quote(session.matchId) .. " slot=" .. quote(saveSlot)
    )
    if type(Isaac1v1P2P.VerifyCompetitiveSaveTarget) ~= "function" then
        return failConsumedLaunch(
            "COMPETITIVE SAVE INIT FAILED", "COMPETITIVE_SAVE_TARGET_VERIFY_UNAVAILABLE",
            session, liveIPC, competitiveRun)
    end
    local targetOk, targetValid, targetError = pcall(
        Isaac1v1P2P.VerifyCompetitiveSaveTarget, "AFTER_START_NEW_GAME")
...
local function confirmStarted(session, gameState, matchValidation, competitiveRun)
    ...
    lastError = nil
    Isaac.DebugString(
        "[Isaac1v1P2P] RUN_LAUNCH_STARTED match_id=" ..
        quote(session.matchId or "Unknown")
    )
    Isaac.DebugString(
        "[Isaac1v1P2P] RUN_START_EXECUTED match_id=" ..
        quote(session.matchId or "Unknown")
    )
    Isaac.DebugString(
        "[Isaac1v1P2P] COMPETITIVE_RUN_STATE active=true match_id=" ..
        quote(session.matchId or "Unknown")
    )
end
...
function runLauncher.Register(mod, matchSession, gameState, matchValidation, matchBridge, liveIPC, competitiveRun, modCompatibility)
    ...
    mod:AddCallback(ModCallbacks.MC_MAIN_MENU_RENDER, function()
        ...
        if not triggerLogged then
            triggerLogged = true
            Isaac.DebugString("[Isaac1v1P2P] RUN_LAUNCH_MAIN_MENU_TRIGGER")
        end
        ...
        if liveStart ~= nil and liveIPC ~= nil and liveIPC.IsCancelled ~= nil then
            ...
            if cancelledOk and cancelled == true then
                Isaac.DebugString("[Isaac1v1P2P] RUN_START_CANCELLED_IGNORED match_id=" .. quote(liveStart.matchId))
                return
            end
        end
        ...
        if launchRequest == nil or launchRequest.requested ~= true then
            if not idleLogged then
                idleLogged = true
                Isaac.DebugString("[Isaac1v1P2P] RUN_LAUNCH_IDLE reason=\"NO_LAUNCH_REQUEST\"")
            end
            return
        end
        attemptedMatchKey = startKey
        Isaac.DebugString("[Isaac1v1P2P] MATCH_SESSION_APPLIED match_id=" .. quote(session.matchId)
            .. " generation=" .. quote(liveStart ~= nil and liveStart.startGeneration or "<none>"))
    ...
    mod:AddCallback(ModCallbacks.MC_POST_GAME_STARTED, function()
        ...
        if session ~= nil and requestedMatchId == session.matchId then
            local targetOk, targetValid, targetError = pcall(
                Isaac1v1P2P.VerifyCompetitiveSaveTarget, "POST_GAME_STARTED")
            if not targetOk or targetValid ~= true then
                failConsumedLaunch(
                    "COMPETITIVE SAVE INIT FAILED",
                    "COMPETITIVE_SAVE_PATH_INIT_FAILED: "
        ...
        if liveIPC ~= nil and liveIPC.AcknowledgeStarted ~= nil and session ~= nil
            ...
            if generation ~= nil then
                ...
                if ackOk and ackSent == true then
                    Isaac.DebugString(
                        "[Isaac1v1P2P] MATCH_STARTED_ACK_SENT " ..
    ...
    Isaac.DebugString("[Isaac1v1P2P] Run launcher initialized")
end
