...
local function logSelection(option)
    Isaac.DebugString("[Isaac1v1] MENU_1V1_SELECTION option=\"" .. option .. "\"")
end
        ...
        if layer ~= nil and type(layer.IsVisible) == "function"
            ...
            if alreadyRecorded then
                ...
                if visibleOk then
                    ...
                    Isaac.DebugString("[Isaac1v1] MENU_1V1_BESTIARY_LAYER source=\""
...
local function captureHostLayers()
    ...
    if captured then
        Isaac.DebugString("[Isaac1v1] MENU_1V1_HOST_UI_HIDDEN layers="
...
local function captureAndHideRepentogonBestiaryExtras()
    ...
    if hostExtrasCaptured then
        Isaac.DebugString("[Isaac1v1] MENU_1V1_REPENTOGON_BESTIARY_EXTRAS_HIDDEN fields="
...
local function restoreHostLayers()
    ...
    Isaac.DebugString("[Isaac1v1] MENU_1V1_HOST_UI_RESTORED")
end
...
local function enterContext()
    ...
    if inputMask == nil then
        Isaac.DebugString("[Isaac1v1] ERROR: 1v1 menu input mask unavailable")
        return false
    end
    ...
    if not menuOk then
        Isaac.DebugString("[Isaac1v1] ERROR: unable to enter 1v1 menu context")
        previousMenu = nil
        previousInputMask = nil
        return false
    end
    ...
    if not maskOk then
        ...
        Isaac.DebugString("[Isaac1v1] ERROR: unable to own 1v1 menu input")
        previousMenu = nil
        previousInputMask = nil
        return false
    end
    ...
    Isaac.DebugString("[Isaac1v1] MENU_1V1_CONTEXT_ENTER from=\""
        .. menuName(previousMenu) .. "\" to=\"" .. menuName(HOST_MENU) .. "\"")
    Isaac.DebugString("[Isaac1v1] MENU_1V1_OPENED state=\"MAIN_1V1\" selection=1")
    return true
end
...
local function exitContext()
    ...
    Isaac.DebugString("[Isaac1v1] MENU_1V1_CONTEXT_EXIT to=\""
        .. menuName(targetMenu) .. "\"")
    previousMenu = nil
    previousInputMask = nil
end
...
local function beginFindMatch()
    ...
    if joined and sent == true then
        ...
        selection = 1
        Isaac.DebugString("[Isaac1v1] MENU_FIND_MATCH")
    else
        pendingError = errorMessage or "COMPANION_NOT_RUNNING"
        state = STATE_ERROR
    end
end
...
local function back()
    ...
    elseif state == STATE_CANCELLING then
        ...
        state = STATE_MAIN
        selection = 1
        Isaac.DebugString("[Isaac1v1] MENU_1V1_STATE state=\"MAIN_1V1\"")
    end
end
...
local function activateSelection()
    ...
    if state == STATE_MAIN then
        if option == "FIND MATCH" then
            ...
            if not statusOk or type(status) ~= "table"
                or status.companion ~= "CONNECTED" then
                pendingError = "COMPANION_NOT_RUNNING"
                state = STATE_ERROR
                selection = 1
                return
            end
        ...
        if option == "READY" then
            if liveIPC == nil or type(liveIPC.Ready) ~= "function" then
                ...
                if not ok or sent ~= true then
                    pendingError = errorMessage or "BACKEND_UNAVAILABLE"
                    state = STATE_ERROR
                else
                    state = STATE_STARTING
                    Isaac.DebugString("[Isaac1v1] MATCH_READY_SENT")
                end
            end
        else
            requestCancel()
    ...
    elseif state == STATE_RESULT then
        ...
        elseif option == "BACK TO 1V1" then
            ...
            state = STATE_MAIN
            selection = 1
            Isaac.DebugString("[Isaac1v1] MATCH_RESULT_BACK_TO_1V1")
        else
            clearResult()
            exitContext()
        end
...
requestCancel = function()
    ...
    if cancellingState == STATE_MATCH_FOUND or cancellingState == STATE_STARTING then
        Isaac.DebugString("[Isaac1v1] MENU_MATCH_CANCEL_REQUEST state=\"" .. cancellingState .. "\"")
    elseif cancellingState == STATE_SEARCHING then
        Isaac.DebugString("[Isaac1v1] MENU_QUEUE_CANCEL_REQUEST state=\"SEARCHING\"")
    end
    ...
    if leaveOk and sent == true then
        ...
        pendingError = leaveError or "COMPANION_NOT_RUNNING"
        state = STATE_ERROR
        selection = 1
    end
end
...
local function renderAvatar(player, x, y)
    ...
    local safeOk, safe = pcall(Isaac1v1IPC.IsSteamAvatarPath, path)
    ...
    if avatarPathLoaded ~= path then
        ...
        avatarLoaded = loaded
        if not loaded then Isaac.DebugString("[Isaac1v1] STEAM_AVATAR_UNAVAILABLE reason=\"SPRITE_LOAD_FAILED\"") end
    end
...
local function renderDedicatedScreen()
    ...
    elseif state == STATE_CANCELLING and ipcStatus.queue == "IDLE" then
        ...
        selection = 1
        Isaac.DebugString("[Isaac1v1] MENU_1V1_STATE state=\"MAIN_1V1\"")
        Isaac.DebugString("[Isaac1v1] QUEUE_CANCELLED")
        Isaac.DebugString("[Isaac1v1] MENU_LEAVE_QUEUE_COMPLETE")
    ...
    if state == STATE_CANCELLING and liveIPC ~= nil and type(liveIPC.GetLeaveElapsed) == "function" then
        ...
        if elapsedOk and type(elapsed) == "number" and elapsed >= 7 then
            ...
            selection = 1
            Isaac.DebugString("[Isaac1v1] QUEUE_LEAVE_TIMEOUT elapsed=\"" .. tostring(math.floor(elapsed)) .. "\"")
        end
    end
    ...
    elseif state == STATE_CANCELLING then
        renderText("WAITING FOR SERVER", 180, 116, color)
    elseif state == STATE_MATCH_FOUND then
    ...
    elseif state == STATE_STARTING then
        renderText("RECEIVING AUTHORITATIVE SESSION", 140, 144, color)
    elseif state == STATE_ERROR then
        local message = (pendingError == "BACKEND_UNAVAILABLE"
            or pendingError == "BACKEND_TIMEOUT"
            or pendingError == "BACKEND_HTTP_ERROR"
            or pendingError == "BACKEND_INVALID_RESPONSE") and "SERVER UNAVAILABLE"
            or pendingError == "IPC_COMPONENT_NOT_AVAILABLE" and "CONNECTION UNAVAILABLE"
            or pendingError == "COMPANION_NOT_RUNNING" and "COMPANION NOT RUNNING"
    ...
    elseif state == STATE_MAIN then
        if ipcStatus.component ~= "INSTALLED" or ipcStatus.companion ~= "CONNECTED" then
            renderText("CONNECTION UNAVAILABLE", 164, 72, selectedColor)
        elseif ipcStatus.backend ~= "CONNECTED" then
            renderText("SERVER UNAVAILABLE", 178, 72, selectedColor)
        else
    ...
    elseif state == STATE_RESULT then
        ...
        if resultMappingLoggedMatchId ~= result.matchId then
            ...
            Isaac.DebugString("[Isaac1v1] RESULT_MAPPING local_player_id=\"" .. tostring(playerId)
...
function menuPrototype.ShowResult(result)
    ...
    Isaac.DebugString("[Isaac1v1] MATCH_RESULT_MENU_PENDING match_id=\"" .. result.matchId .. "\"")
    return true
end
...
local function renderPrototype()
    ...
    if not active then
        if resultSnapshot ~= nil and currentMenu == MainMenuType.GAME then
            Isaac.DebugString("[Isaac1v1] MAIN_MENU_CONTEXT_READY menu=\"GAME\"")
            if enterContext() then
                state = STATE_RESULT
                selection = 1
                Isaac.DebugString("[Isaac1v1] RESULT_MENU_OPENED match_id=\"" .. tostring(resultSnapshot.matchId) .. "\"")
            end
            return
        end
    ...
    if currentMenu ~= HOST_MENU then
        Isaac.DebugString("[Isaac1v1] ERROR: 1v1 menu context ownership lost")
        exitContext()
        return
    end
...
function menuPrototype.Register(mod, bridge, ipc, compatibility, launcher)
    ...
    if Font == nil or Sprite == nil then
        Isaac.DebugString("[Isaac1v1] ERROR: dedicated 1v1 menu rendering API unavailable")
        return
    end
    ...
    if HOST_MENU == nil or ButtonActionBitwise == nil
        ...
        Isaac.DebugString("[Isaac1v1] ERROR: dedicated 1v1 menu context API unavailable")
        return
    end
    ...
    if not fontOk or not fontLoaded then
        Isaac.DebugString("[Isaac1v1] ERROR: dedicated 1v1 menu font unavailable")
    end
    ...
    if not paperOk or not paperLoaded or not backdropLoaded or not mainEntryLoaded then
        Isaac.DebugString("[Isaac1v1] ERROR: dedicated 1v1 paper asset unavailable")
    end

    if ModCallbacks.MC_MAIN_MENU_RENDER == nil
        or ModCallbacks.MC_MENU_INPUT_ACTION == nil then
        Isaac.DebugString("[Isaac1v1] ERROR: dedicated 1v1 menu callbacks unavailable")
        return
    end

    mod:AddCallback(ModCallbacks.MC_MAIN_MENU_RENDER, renderPrototype)
    mod:AddCallback(ModCallbacks.MC_MENU_INPUT_ACTION, onMenuInput)
    if localBridge == nil then
        Isaac.DebugString("[Isaac1v1] ERROR: in-game menu local bridge unavailable")
    end
    Isaac.DebugString("[Isaac1v1] Dedicated 1v1 menu initialized host=\"ISAAC_1V1\"")
end
